...
def search_hyperparam(trial: optuna.trial.Trial) -> Dict[str, Any]:
    ...
    epochs = trial.suggest_int("epochs", low=140, high=200, step=10)
    img_size = trial.suggest_categorical("img_size", [96, 112, 168, 224])
    n_select = trial.suggest_int("n_select", low=0, high=8, step=2)
    batch_size = trial.suggest_int("batch_size", low=16, high=64, step=16)
    # caution => add optimizer
...
def objective(trial: optuna.trial.Trial, device) -> Tuple[float, float, float]:
    """Optuna objective.
    Args:
        trial
    Returns:
    ...
    # caution => lb metric
    # final_score = get_metric(f1_score, mean_time)
    # return final_score
    return f1_score, params_nums, mean_time

# caution => mean_time != lb time (guess)
def get_metric(f1_score: float, mean_time: float) -> float:
    '''
    A function for calculating lb score
    '''
    score_submittime = mean_time / 60
    score_f1 = 1 / (1 + np.exp(-(20 * (0.7 - f1_score))))
    final_score = 0.5 * score_submittime + score_f1
...
def tune(gpu_id, storage: str = None):
    ...
    sampler = optuna.samplers.MOTPESampler() # caution => TPE
...
if __name__ == "__main__":
    ...
    # caution => write down if you wanna connect db
    parser.add_argument("--storage", default="", type=str, help="Optuna database storage path.")
    args = parser.parse_args()

    study_name = 'example'
    args.storage = f"sqlite:///{study_name}.db"
    tune(args.gpu, storage=args.storage if args.storage != "" else None)

    # caution => check sqlite db
    # study = optuna.create_study(study_name=study_name, storage=args.storage, load_if_exists=True)
